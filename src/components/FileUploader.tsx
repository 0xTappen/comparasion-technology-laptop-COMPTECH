...
export default function FileUploader({ onDataLoaded, dataCount, isLoading, setIsLoading }: Props) {
  ...
  const processFile = useCallback(
    async (buffer: ArrayBuffer) => {
      ...
      try {
        ...
        alert("Failed to process the Excel file. Make sure the file format is valid.");
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handleLoadDefault = useCallback(async () => {
    ...
    try {
      ...
      alert("The default dataset file was not found in the /dataset folder.");
      setIsLoading(false);
    }
  ...
  return (
    <div className="brutal-card bg-white p-6 md:p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className="font-bold text-xl">Dataset Input</h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <Upload size={32} />
          <span className="font-bold text-lg">Upload Dataset File</span>
          <span className="text-sm opacity-70">Choose an `.xlsx` file from your device</span>
        </button>
        <input
          ref={fileInputRef}
          type="file"
        ...
        >
          <Database size={32} />
          <span className="font-bold text-lg">Use Default Dataset</span>
          <span className="text-sm opacity-70">Loads 1,000 laptop alternatives</span>
        </button>
      </div>
      ...
      {dataCount > 0 && !isLoading && (
        <div className="mt-4 bg-brutal-green border-3 border-black p-3 flex items-center gap-2 font-bold text-sm shadow-[4px_4px_0px_rgba(0,0,0,1)]">
          ...
          laptops loaded and processed successfully.
        </div>
      )}
    </div>
  );
